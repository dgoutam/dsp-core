 ...
 * The base class for DSP system models
 *
...
abstract class BaseDspSystemModel extends BaseDspModel
{
	...
	{
		return array(
			'created_by'       => array( self::BELONGS_TO, 'DreamFactory\\Yii\\Models\\DspUser', 'created_by_id' ),
			'last_modified_by' => array( self::BELONGS_TO, 'DreamFactory\\Yii\\Models\\DspUser', 'last_modified_by_id' ),
		);
	}
	...
	protected function assignManyToOne( $one_id, $many_table, $many_field, $many_records = array() )
	{
		if ( empty( $one_id ) )
		{
			throw new \InvalidArgumentException( "The id can not be empty.", ErrorCodes::BAD_REQUEST );
		}
