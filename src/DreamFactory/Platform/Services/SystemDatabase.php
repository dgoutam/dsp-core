...
/**
 * SystemDatabase.php
 * Provides system database services to the DSP
 *
...
class SystemDatabase extends BaseResourceService
{
	 ...
	 * @var
	 */
	protected static $_modelId;
	/**
	 * @var
	 */
	protected static $_relatedModelName;
	...
	public function getState()
	{
		try
		{
			...
			$_tables = $_db->schema->getTableNames();
			...
			$_contents = file_get_contents( dirname( dirname( Pii::basePath() ) ) . '/config/database/system_schema.json' );

			if ( !empty( $_contents ) )
			{
				$_contents = Utilities::jsonToArray( $_contents );
				// check for any missing necessary tables
				$needed = Utilities::getArrayValue( 'table', $_contents, array() );
				foreach ( $needed as $table )
				{
					$name = Utilities::getArrayValue( 'name', $table, '' );
					if ( !empty( $name ) && !in_array( $name, $_tables ) )
					{
						return 'schema required';
					}
				}
				$version = Utilities::getArrayValue( 'version', $_contents );
				$oldVersion = '';
	...
	public function initSchema()
	{
		try
		{
			$_contents = file_get_contents( Yii::app()->basePath . '/data/system_schema.json' );
			if ( empty( $_contents ) )
			{
			...
			$_tables = Utilities::getArrayValue( 'table', $_contents );
			if ( empty( $_tables ) )
			{
				throw new \Exception( "No default system schema found." );
			}
			$result = DbUtilities::createTables( $_db, $_tables, true, false );
...
//            if ((empty($result)) || !isset($result[0]['ROUTINE_NAME'])) {
			switch ( DbUtilities::getDbDriverType( $_db ) )
			{
				case DbUtilities::DRV_SQLSRV:
					$_contents = file_get_contents( Yii::app()->basePath . '/data/procedures.mssql.sql' );
					if ( ( false === $_contents ) || empty( $_contents ) )
					{
					...
					$command->execute();
					$command->reset();
					$command->setText( $_contents );
					$command->execute();
					break;
				case DbUtilities::DRV_MYSQL:
					$_contents = file_get_contents( Yii::app()->basePath . '/data/procedures.mysql.sql' );
					if ( ( false === $_contents ) || empty( $_contents ) )
					{
						throw new \Exception( "Empty or no system db procedures file found." );
					}
...
//            }
	...
	public function initData()
	{
		try
		{
			...
			$_contents = file_get_contents( Yii::app()->basePath . '/data/system_data.json' );
			if ( empty( $_contents ) )
			{
			...
			if ( empty( $result ) )
			{
				$services = Utilities::getArrayValue( 'df_sys_service', $_contents );
				if ( empty( $services ) )
				{
			...
			if ( empty( $result ) )
			{
				$apps = Utilities::getArrayValue( 'df_sys_app', $_contents );
				if ( !empty( $apps ) )
				{
					foreach ( $apps as $app )
					{
		...
		try
		{
			...
			switch ( $this->_modelName )
			{
				...
				case 'user':
					...
					if ( empty( $this->_modelId ) )
					{
						$ids = Utilities::getArrayValue( 'ids', $_REQUEST, '' );
						if ( !empty( $ids ) )
						{
							$result = $this->retrieveRecordsByIds( $this->_modelName, $ids, $fields, $extras );
						}
						else
							...
							{ // complex filters or large numbers of ids require post
								...
								else
								{
									...
									if ( !empty( $records ) )
									{
										...
										$result = $this->retrieveRecords( $this->_modelName, $records, $fields, $extras );
									}
									else
					...
					{ // single entity by id
						$result = $this->retrieveRecordById( $this->_modelName, $this->_modelId, $fields, $extras );
					}
					break;
				default:
					throw new Exception( "GET request received for an unsupported system resource named '$this->_modelName'.", ErrorCodes::BAD_REQUEST );
					break;
			}
			...
			switch ( $this->_modelName )
			{
				...
				case 'user':
					...
					if ( empty( $records ) )
					{
						...
						$result = $this->createRecord( $this->_modelName, $data, $fields, $extras );
					}
					else
					{
						...
						$result = $this->createRecords( $this->_modelName, $records, $rollback, $fields, $extras );
					}
					break;
				default:
					throw new Exception( "POST request received for an unsupported system resource named '$this->_modelName'.", ErrorCodes::BAD_REQUEST );
					break;
			}
			...
			switch ( $this->_modelName )
			{
				...
				case 'user':
					if ( empty( $this->_modelId ) )
					{
						...
						if ( !empty( $ids ) )
						{
							$result = $this->updateRecordsByIds( $this->_modelName, $ids, $data, $rollback, $fields, $extras );
						}
						else
						{
							$records = Utilities::getArrayValue( 'record', $data, null );
							...
							if ( empty( $records ) )
							{
								...
								$result = $this->updateRecord( $this->_modelName, $data, $fields, $extras );
							}
							else
							{
								$result = $this->updateRecords( $this->_modelName, $records, $rollback, $fields, $extras );
							}
						}
					}
					else
					{
						$result = $this->updateRecordById( $this->_modelName, $this->_modelId, $data, $fields, $extras );
					}
					break;
				default:
					throw new Exception( "PUT request received for an unsupported system resource named '$this->_modelName'.", ErrorCodes::BAD_REQUEST );
					break;
			}
		...
		try
		{
			...
			switch ( $this->_modelName )
			{
				...
				case 'user':
					...
					break;
				default:
					throw new Exception( "MERGE/PATCH request received for an unsupported system resource named '$this->_modelName'.", ErrorCodes::BAD_REQUEST );
					break;
			}
			...
			switch ( $this->_modelName )
			{
				...
				case 'user':
					if ( empty( $this->_modelId ) )
					{
						...
						if ( !empty( $ids ) )
						{
							$result = $this->deleteRecordsByIds( $this->_modelName, $ids, $fields, $extras );
						}
						else
						{
							$records = Utilities::getArrayValue( 'record', $data, null );
							...
							if ( empty( $records ) )
							{
								if ( empty( $data ) )
								{
									throw new Exception( "Id list or record containing Id field required to delete $this->_modelName records.", ErrorCodes::BAD_REQUEST );
								}
								$result = $this->deleteRecord( $this->_modelName, $data, $fields, $extras );
							}
							else
							{
								$result = $this->deleteRecords( $this->_modelName, $records, $fields, $extras );
							}
						}
					}
					else
					{
						$result = $this->deleteRecordById( $this->_modelName, $this->_modelId, $fields, $extras );
					}
					break;
				default:
					throw new Exception( "DELETE request received for an unsupported system resource named '$this->_modelName'.", ErrorCodes::BAD_REQUEST );
					break;
			}
	...
	protected function detectCommonParams()
	{
		...
		$this->_modelName = strtolower( ( isset( $resource[0] ) ) ? $resource[0] : '' );
		$this->_modelId = ( isset( $resource[1] ) ) ? $resource[1] : '';
	}
											 ...
											 $extras = array() )
	{
		...
		try
		{
			...
			if ( $include_count || $include_schema )
			{
				...
				if ( $include_schema )
				{
					$results['meta']['schema'] = \DbUtilities::describeTable( Pii::db(), $model->tableName(), static::SYSTEM_TABLE_PREFIX );
				}
			}
