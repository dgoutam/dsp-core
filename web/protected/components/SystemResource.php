	...
	protected function _handleAction()
	{
		...
		switch ( $this->_action )
		{
			case self::Get:
				...
				{
					if ( 0 == strcasecmp( 'app', $this->_apiName ) )
					{
				...
				if ( empty( $this->_resourceId ) )
				{
					...
					else
					{
						...
						if ( empty( $records ) )
						{
							if ( empty( $data ) )
							{
								throw new Exception( "Id list or record containing Id field required to delete $this->_apiName records.", ErrorCodes::BAD_REQUEST );
							}
